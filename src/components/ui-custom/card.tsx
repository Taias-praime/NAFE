    ...
    return (
        <div className={cn("bg-background/70 min-h-[200px] w-full px-5 2xl:px-10 py-5 flex items-center justify-between overflow-hidden rounded", className)}>
            <div className="z-10">
                ...
                {cta && <Button className="p-0" size={"sm"}> <AddEvent setReload={setReload} setIsEditEvent={() => {}} isEditEvent={false} currentStep={currentStep} className="flex gap-3 p-3"> {ctaIcon} {cta} </AddEvent> </Button>}
